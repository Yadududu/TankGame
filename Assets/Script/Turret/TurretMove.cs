	...
	public class TurretMove : BaseMove {
		...
		public override void MoveMethod(){
			...
			if (Mathf.Abs(mouseX) > Mathf.Abs(mouseY)){
				Camera3D.transform.eulerAngles += new Vector3(0, mouseX, 0);
				transform.localRotation = transform.localRotation * Quaternion.Euler(0, mouseX, 0);
			}
			else{
				Camera3D.transform.eulerAngles += new Vector3(-mouseY, 0, 0);//摄像机绕x轴旋转的方向跟鼠标y移动方向相反
				// Debug.Log(Camera3D.transform.eulerAngles.x);
				if(Camera3D.transform.eulerAngles.x <= 350 & Camera3D.transform.eulerAngles.x >= 270){
					Camera3D.transform.rotation = Quaternion.Euler(new Vector3(-10,Camera3D.transform.eulerAngles.y,0));
				}
				if(Camera3D.transform.eulerAngles.x >= 45 & Camera3D.transform.eulerAngles.x <= 90){
					Camera3D.transform.rotation = Quaternion.Euler(new Vector3(45,Camera3D.transform.eulerAngles.y,0));
				}
			}
